...
public class Office {
    ...
    public void readDataFromFiles() throws IOException, ClassNotFoundException {
        ...
        readTripsFromFile("trips.bin");
        readGuidesFromFile("guides.bin");
        readClientsFromFile("clients.bin");
    }
     ...
     */
    public void addTrip(String country, String city, double price, int maxSeats, String begin, String end, TourGuide tg, CateringInfo cI) {
        int id = 0;
        if (trips.size() != 0) {
            id = trips.size();
        }
        //Creating a new Trip object with data from parameters of the method
        Trip newTrip = new Trip(id, maxSeats, 0, country, city, begin, end, price, tg, cI);
